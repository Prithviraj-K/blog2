...
`ls -h {}`: Uses the `ls` command to list information about the found files. The `-h` option is used to make file sizes more human-readable. The `{}` are replaced by the matched file for each iteration.
...
### Linpeas Results
The linpeas results showed php7.4 binary file in the directory `/usr/bin/php7.4` that does has an unknown SUID binary. The output is shown below
```
...
- Using this information we can try to find privilege escalation for SUID in [GTFOBins](https://gtfobins.github.io/gtfobins/php/)

GTFOBins uses the following command
``` bash
