...
const heap = require('./utils/heap.cjs');
            ...
            for (const value of values) {
                if (Array.isArray(value)) {
                    for (const item of value) {
                        if (misc.isPlainObject(item)) {
                            addObjectKeysToSet(item, keys);
                        }
                    }
                } else if (misc.isPlainObject(value)) {
...
const createStatApi = (source, { value, stats, assertions }) => ({
    get value() {
        return value;
    },
    ...
    suggestion(pos, options) {
        ...
        sort = normalizeFunctionOption(sort, compileBuildin.cmp);
        filterFactory = normalizeFunctionOption(filterFactory, defaultFilterFactory);

        const storageType = sort && isFinite(limit) ? heap.MaxHeap : Set;
