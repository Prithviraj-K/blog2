...
function compile(ast, tolerant = false, suggestions = null) {
    ...
    function walk(node, relatedNode) {
        ...
        if (index.compile.has(node.type)) {
            index.compile.get(node.type)(node, ctx, relatedNode);
        } else {
        ...
        if (usedAssertions.size) {
            ...
            initCtx.assertAssertions = function(providedAssertions) {
                for (const [assertion, range] of usedAssertions.entries()) {
                    if (!misc.hasOwn(providedAssertions, assertion)) {
                        return () => {
                            throw Object.assign(
        ...
        if (usedMethods.size) {
            ...
            initCtx.assertMethods = function(providedMethods) {
                for (const [method, range] of usedMethods.entries()) {
                    if (!misc.hasOwn(providedMethods, method)) {
                        return () => {
                            throw Object.assign(
                                new Error(
    ...
    try {
        ...
        throw error('SyntaxError', 'Jora query compilation error', {
